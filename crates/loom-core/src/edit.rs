...
pub fn fix_for(project: &Project, finding: &Finding) -> Option<Fix> {
    match finding.rule {
        ...
        //
        // 這幾條都沒有「填一格」的修法，但下一步各自不同——
        // 寫死一句「要改接」的話，對 L013 與 L014 都是錯的。
        Rule::L003 => Some(Fix::Manual {
            hint: "要改接：這條連線指到了不存在的東西".into(),
        }),
        Rule::L012 => Some(Fix::Manual {
            hint: "改掉那個參照，或把懸空的元素刪掉".into(),
        }),
        // L013 有**兩個都合理的答案**，工具挑一個等於替使用者決定他的架構。
        Rule::L013 => Some(Fix::Manual {
            hint: "把兩端對調，或把目標改成對方的服務".into(),
        }),
        Rule::L014 => Some(Fix::Manual {
            hint: "改掉其中一個的名字".into(),
        }),
    }
}
