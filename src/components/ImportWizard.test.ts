...
function changes(extra: Partial<Change> = {}): Change {
  return {
    kind: 'added',
...
/** 直接把預覽結果塞進元件，跳過選檔那一步。 */
async function mountWithPlan(p: Plan) {
  const w = mount(ImportWizard)
  ;(w.vm as unknown as { plan: Plan | null }).plan = p
  await w.vm.$nextTick()
  return w
}
...
describe('匯入精靈', () => {
  ...
  it('預覽會分別數出新增與更新', async () => {
    const w = await mountWithPlan(plan({
      changes: [changes(), changes({ label: 'prod／redis-05' }), changes({ kind: 'updated', before: 'a', after: 'b' })],
      unchanged: 12,
    }))
  ...
  it('更新的項目把前後值並排', async () => {
    const w = await mountWithPlan(plan({
      changes: [changes({ kind: 'updated', element: 'address', label: 'prod／redis-01／client-port', before: '10.0.1.2:6379', after: '10.0.1.12:6379' })],
    }))
  ...
  it('會說清楚動到哪些環境', async () => {
    const w = await mountWithPlan(plan({ changes: [changes()], environments: ['prod', 'test'] }))
    expect(w.text()).toContain('prod、test')
  })

  it('沒有任何變更時說明白，而且不讓人按套用', async () => {
    const w = await mountWithPlan(plan({ unchanged: 47 }))
  ...
  it('套用鈕上寫著會套用幾項', async () => {
    ...
    const w = await mountWithPlan(plan({ changes: [changes(), changes({ label: 'x' })] }))
