...
function 提案(extra: Partial<Proposal> = {}): Proposal {
  return {
    id: 'conn-新的',
    serves: 'r-cache',
    purpose: '訂單服務讀寫快取',
    from: 來源[0]!.endpointing,
    to: 目標[0]!.endpointing,
...
describe('補一條連線', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(commands.connectionChoices).mockImplementation(async (_env, end) =>
      ({ status: 'ok', data: end === 'from' ? 來源 : 目標 }) as never,
    )
  })

  async function 開起來(p: Proposal = 提案()) {
    vi.mocked(commands.proposeConnection).mockResolvedValue({ status: 'ok', data: p } as never)
    store.新增連線中 = { environment: 'env-prod', relationship: 'r-cache', label: '沒有任何實際連線' }
  ...
  it('兩個選單都照 Rust 給的分組列出來', async () => {
    const w = await 開起來()
    const 群組 = w.findAll('optgroup').map((g) => g.attributes('label'))
    expect(群組).toContain('服務（整群）')
    expect(群組).toContain('設備')
    expect(群組).toContain('人')
  })
  ...
  it('換掉某一端之後送的是換過的那個', async () => {
    ...
    const 目標選單 = w.findAll('select')[1]!
    await 目標選單.setValue('1')
    await w.find('.primary').trigger('click')

    expect(套用).toHaveBeenCalledWith(
      expect.objectContaining({
        addConnection: expect.objectContaining({ to: 目標[1]!.endpointing }),
      }),
    )
  })
  ...
  it('有一端擬不出來時不能建立，而且說得出為什麼', async () => {
    ...
    const w = await 開起來(提案({ to: null, notes: ['目標的服務 redis 在 prod 一台都還沒建，要先建機器。'] }))
